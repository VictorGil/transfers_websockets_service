 ...
 * since August 2019
 */
public class TransferDataUpdateSender implements TransferSender {
    private static final Logger log = LoggerFactory.getLogger(TransferDataUpdateSender.class);

    private final ObjectMapper mapper = new ObjectMapper();

    private final MessageSender messageSender;

    public TransferDataUpdateSender(MessageSender messageSender) {
    ...
    public void send(TransferEntity transfer, Session session) {
        ...
        MessageWrapper message = new MessageWrapper(MessageType.TRANSFER_DATA_UPDATE.name(),
                json);
